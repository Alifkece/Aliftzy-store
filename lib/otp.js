...
// File wajib ada di assets/aliftzy-otp.gif (16:9). Dibaca sebagai lampiran
// inline (Content-ID) supaya tampil di Gmail tanpa bergantung pada hosting
// eksternal — menggantikan <video> + URL MP4 eksternal yang dipakai sebelumnya.
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OTP_GIF_PATH = path.join(__dirname, "..", "assets", "aliftzy-otp.gif");
const OTP_GIF_CID = "aliftzy-otp-gif";

function loadOtpGifAttachment() {
  try {
    return {
      filename: "aliftzy-otp.gif",
    ...
    console.error(
      "Tidak dapat membaca assets/aliftzy-otp.gif (pastikan file sudah ditambahkan ke repo):",
      err.message
    );
    return null;
  }
...
// Ikon SVG inline (data URI) sebagai pengganti emoji. Dipilih karena data URI
// image lebih konsisten dirender oleh Gmail dibanding <svg> inline mentah;
// kalau pun gagal dirender oleh client tertentu, email tidak rusak — hanya
// ikon yang tidak muncul, teks di sebelahnya tetap terbaca.
function svgIcon(svg) {
  return `data:image/svg+xml;base64,${Buffer.from(svg).toString("base64")}`;
}
const ICON_GREETING = svgIcon(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#16e5ff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-8.5 8.5H3l2-4a8.38 8.38 0 0 1 8.5-13 8.38 8.38 0 0 1 7.5 8.5z"/></svg>'
);
const ICON_CLOCK = svgIcon(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#8a96a8" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg>'
);
const ICON_LOCK = svgIcon(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#aab4c2" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="10" width="16" height="10" rx="2"/><path d="M7 10V7a5 5 0 0 1 10 0v3"/></svg>'
);
const ICON_COPY = svgIcon(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#16e5ff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="4" width="12" height="14" rx="2"/><path d="M4 8v10a2 2 0 0 0 2 2h8"/></svg>'
);
    ...
    <tr>
      <td align="center">
        <table role="presentation" width="100%" style="max-width:420px;background:#0d1420;border:1px solid #16e5ff33;border-radius:20px;overflow:hidden;">
          ...
          <!-- GIF OTP (assets/aliftzy-otp.gif, dilampirkan sebagai inline CID attachment, 16:9 asli tanpa crop/stretch) -->
          <tr>
            <td style="padding:20px 20px 0;">
              <div style="border-radius:14px;overflow:hidden;background:#0a121c;border:1px solid #16e5ff26;">
                <img src="cid:${OTP_GIF_CID}" width="100%" alt="Aliftzy Store" style="display:block;width:100%;height:auto;border:0;background:#0a121c;">
              </div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:22px 32px 0;color:#e8edf5;font-size:15px;line-height:1.65;">
              <img src="${ICON_GREETING}" width="15" height="15" alt="" style="vertical-align:-2px;margin-right:6px;">Halo,<br><br>
                ...
                <tr>
                  <td style="border-radius:10px;background:#16e5ff14;border:1px solid #16e5ff40;">
                    <a href="${otpPageUrl}" target="_blank" style="display:block;padding:11px 26px;font-family:Arial,sans-serif;font-size:12.5px;font-weight:700;letter-spacing:1.5px;color:#16e5ff;text-decoration:none;">
                      <img src="${ICON_COPY}" width="12" height="12" alt="" style="vertical-align:-1px;margin-right:6px;">SALIN KODE
                    </a>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td style="padding:12px 32px 0;text-align:center;font-size:12px;color:#8a96a8;">
              <img src="${ICON_CLOCK}" width="12" height="12" alt="" style="vertical-align:-1px;margin-right:5px;">Kode ini hanya berlaku selama 5 menit.
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:22px 32px 26px;color:#aab4c2;font-size:12.5px;line-height:1.75;">
              ...
              <br><br>
              <img src="${ICON_LOCK}" width="12" height="12" alt="" style="vertical-align:-1px;margin-right:5px;">Kode hanya dapat digunakan satu kali dan otomatis tidak berlaku setelah 5 menit.
            </td>
          </tr>
