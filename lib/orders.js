 ...
 *
 * FIX STOK PER PAKET: sebelumnya stok hanya dicek berdasarkan productId,
 * jadi produk yang stoknya cuma tersedia di satu paket (mis. "1 Tahun")
 * dianggap tersedia juga untuk paket lain (mis. "1 Bulan") yang sebenarnya
 * kosong. Sekarang SELURUH pengecekan & pengambilan stok memakai kombinasi
 * productId + packageName, sama persis dengan yang dipakai Admin saat
 * menambah stok (lihat aliftzy-admin/js/pages/stock.js).
 */

export async function getAvailableStockCount(productId, packageName) {
  const snap = await db
    .collection("stock")
...
export async function validateStockAndDeliver(transactionId, paymentType) {
  ...
  return db.runTransaction(async (t) => {
    ...
    const stockQuery = db
      ...
      .where("packageName", "==", order.packageName)
      .where("sold", "==", false)
      .limit(1);
